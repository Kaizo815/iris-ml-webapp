...
st.caption("Python Course Project | Data Visualization + Classical ML + Web Interaction")

with st.expander("先看这个网页能做什么", expanded=True):
    st.markdown("""
    这个网页把你的大作业整合成了一个可交互演示系统，主要包括四部分：

    1. **Data Overview**：展示数据集基本信息和 Part1 的一些统计结果  
    2. **Visualization**：展示散点图和相关性热力图  
    3. **Model Comparison**：对比四种分类算法的准确率  
    4. **Interactive Model Demo**：选择模型、调参数、查看分类结果  
    """)
...
with tab2:
    ...
    with col1:
        ...
        st.info("结论：Setosa 与另外两类分离明显，Versicolour 和 Virginica 存在一定重叠。")
    with col2:
        st.pyplot(plot_heatmap(df), use_container_width=True)
        st.info("结论：Petal length 与 petal width 相关性很强，花瓣特征对分类更关键。")
...
with tab3:
    ...
    st.success("这张图适合你课堂上先整体讲：先说明四种算法都有效，再切到交互页重点演示 1~2 个模型。")
...
with tab4:
    ...
    if model_name == "KNN":
        ...
        k = st.sidebar.slider("Choose k", min_value=1, max_value=20, value=5, step=1)
        ...
        c_value = st.sidebar.selectbox("Choose C", [0.01, 0.1, 1, 10, 100], index=2)
        ...
        st.markdown("### Random Forest Demo")
        ...
        st.markdown("### XGBoost Demo")
        n_estimators = st.sidebar.slider("XGBoost Trees", min_value=50, max_value=300, value=100, step=10)
        max_depth = st.sidebar.slider("Max Depth", min_value=2, max_value=8, value=3, step=1)
...
st.sidebar.markdown("---")
st.sidebar.info("课堂展示建议：先点 Model Comparison，再去 Interactive Model Demo 现场调一个参数。")
